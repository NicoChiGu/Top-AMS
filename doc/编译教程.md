# 编译教程
可以使用 ESP-IDF 命令行或者 VScode 进行编译

## 使用ESP-IDF命令行编译
1. 参照 [乐鑫官方教程](https://docs.espressif.com/projects/esp-idf/zh_CN/stable/esp32c3/get-started/windows-setup.html) 安装好ESP-IDF
2. 使用ESP-IDF命令行进入项目目录,如:
    ```bash
    cd 项目目录\Top-AMS
    ```
3. 设置好目标芯片类型,如 **ESP32-C3** :
    ```bash
    idf.py set-target esp32c3
    ```
4. 参照 [sdkconfig配置](./sdkconfig配置.md) 进行menuconfig配置
    ```base
    idf.py menuconfig
    ```
    如果你的芯片就是 **ESP32-C3** ,项目里已经有配置好的 `sdkconfig` 文件,将 `sdkconfig_esp32C3` 改名为 `sdkconfig` 即可
5. 填写配置信息.将 `config_examle.hpp` 改为为 `config.hpp` ,并在用户配置区填写好你的硬件信息等
6. 编译项目
    ```bash
    idf.py build
    ```
    首次编译耗时会比较长
    
    **可选：指定电机通道配置**
    
    如果需要编译特定的电机通道版本，可以通过 CMake 参数指定：
    
    - 编译 4 通道版本：
      ```bash
      idf.py build -DMOTOR_CONFIG=MOTORS_4
      ```
    
    - 编译 6 通道版本：
      ```bash
      idf.py build -DMOTOR_CONFIG=MOTORS_6
      ```
    
    - 编译 8 通道版本（默认）：
      ```bash
      idf.py build -DMOTOR_CONFIG=MOTORS_8
      ```
      或直接使用 `idf.py build`（不传参数默认为 8 通道）
7. 连接设备并烧录
   ```bash
   idf.py -p PORT flash
   ```
   **PORT** 为设备管理器里看到的设备串口号,如 **COM3**
8. 调试设备
   ```bash
    idf.py -p PORT monitor
    ```
    设备运行后就可以在看到设备运行的调试信息  
    
## 使用 GitHub Actions 手动编译

仓库提供 `Manual firmware build and release` 工作流，无需在本地安装 ESP-IDF：

1. 打开 GitHub 仓库的 **Actions** 页面。
2. 在左侧选择 **Manual firmware build and release**。
3. 点击 **Run workflow**，选择需要编译的版本：
   - `all`：同时编译 4、6、8 通道版本。
   - `MOTORS_4`：只编译 4 通道版本。
   - `MOTORS_6`：只编译 6 通道版本。
   - `MOTORS_8`：只编译 8 通道版本。
4. 如果只需要临时编译，保持 **Publish the compiled firmware to GitHub Releases** 未勾选。
5. 如果需要发布到 GitHub Releases：
   - 勾选 **Publish the compiled firmware to GitHub Releases**。
   - 在 **Release tag** 中填写以 `v` 开头且不含空格的版本标签，例如 `v1.0.0`。
6. 等待任务完成：
   - 所有编译都会在该次运行页面底部生成 **Artifacts**。
   - 勾选发布时，所选版本的合并固件还会发布到仓库的 **Releases** 页面，并在任务摘要中显示链接。
7. Artifact 压缩包中的 `Top-AMS-MOTORS_*.bin` 是从地址 `0x0` 写入的合并固件；同时保留了应用、Bootloader、分区表和 `flasher_args.json` 供开发调试。

> GitHub 只会在默认分支存在该工作流文件时显示 **Run workflow** 按钮。构建产物默认保留 14 天；只有主动勾选发布并提供有效标签时才会创建或更新 GitHub Release。

## 使用VScode进行编译
1. 下载[VScode](https://code.visualstudio.com/download)
2. 安装EDF-IDF插件  
按下Ctrl+Shift+X调出扩展,搜索esp-idf并安装
3. 


也可以参考[视频教程](https://www.bilibili.com/video/BV1mm4y1P7y7)
