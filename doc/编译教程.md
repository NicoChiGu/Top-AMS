...
仓库提供 `Manual firmware build` 工作流，无需在本地安装 ESP-IDF：

1. 打开 GitHub 仓库的 **Actions** 页面。
2. 在左侧选择 **Manual firmware build**。
3. 点击 **Run workflow**，选择需要编译的版本：
   - `all`：同时编译 4、6、8 通道版本。
   - `MOTORS_4`：只编译 4 通道版本。
   - `MOTORS_6`：只编译 6 通道版本。
   - `MOTORS_8`：只编译 8 通道版本。
4. 等待任务完成，在该次运行页面底部的 **Artifacts** 中下载对应压缩包。
5. 压缩包中的 `Top-AMS-MOTORS_*.bin` 是从地址 `0x0` 写入的合并固件；同时保留了应用、Bootloader、分区表和 `flasher_args.json` 供开发调试。

> GitHub 只会在默认分支存在该工作流文件时显示 **Run workflow** 按钮。构建产物默认保留 14 天，手动编译不会创建 GitHub Release。
